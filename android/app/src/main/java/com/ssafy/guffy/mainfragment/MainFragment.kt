...
class MainFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            Log.d(TAG, "onCreate: username: $username, userId: $userId")
        }
    ...
    private fun initData() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            friendList = mutableListOf<FriendItemDto>() // 리사이클러뷰에 들어갈 아이템 리스트 초기화
...
//        retrofitInterface.friendsList(userEmail)
                ...
                val dialog = ConfirmDialog(object: ConfirmDialog.ConfirmDialogInterface {
                    override fun onYesButtonClick(id: String) {
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            ...
                            withContext(Dispatchers.Main){ // 그리고 리스트, Adapter에서 삭제하기
    ...
    private fun deleteFirebase(roomId: Int) {
        ...
        var thisChattingRoomRef : DatabaseReference = Firebase.database.getReference("chattingRoomId").child(roomId.toString())
